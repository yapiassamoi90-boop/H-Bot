...
async def rappel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except: await update.message.reply_text("Format: /rappel 20:00 Ton texte chef")
...
async def liste(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data.data: await update.message.reply_text("Aucun rappel actif chef"); return
    msg = "📋 Tes rappels:\n\n"
    for r in data.data: msg += f"ID {r['id']} | {r['heure']} → {r['texte']}\n"
    await update.message.reply_text(msg)

async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        rappel_id = int(context.args[0]); user_id = update.effective_user.id
